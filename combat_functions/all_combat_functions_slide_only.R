...
adjust_vals_slide_only = function(channel,
                       slide_var,
                       #image_var,
                       uid_var,
                       h,
  ...
  chan = as.data.frame(h)[,c(uid_var,slide_var,channel)]
  chan[,channel] = log(chan[,channel]+1)
  ...
  delta_stars = update_delta2(batch_chan, beta_c, omega_c,channel,slide_var=slide_var)
  check_delta_conv = delta_conv(batch_chan, delta_stars,slide_var=slide_var)
...
run_full_combat_slide_only = function(data,
                           save_path,
                           vars_to_adjust,
                           slide_var,
                           #image_var,
  ...
  for (i in 1:length(vars_to_adjust)){
    ...
    chan_i = adjust_vals_slide_only(channel=vars_to_adjust[i],
                         slide_var = slide_var,
                         #image_var = image_var,
                         uid_var = uid_var,
                         tol=tol,
    ...
    h_cb[,paste0(vars_to_adjust[i],'_combat_slide_adjusted')] = chan_i$Y_ijc_star
